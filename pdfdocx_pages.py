# 소스폴더 내 모든 pdf, docx, (doc 파일 제외), 페이지 수 출력
...
# 엑셀 저장 경로
excel_path = os.path.join(target_folder, 'filelist_withpage_ar.xlsx')
...
for root, dirs, files in os.walk(source_folder):
    ...
    if '@' in root:
        continue
    for file in files:
        # '@'가 파일명에 포함되지 않으며, 확장자가 '.pdf', '.PDF', '.docx', '.DOCX'인 경우
...
# 에러가 발생한 파일 경로 출력
if error_files_pdf or error_files_docx:
    print("\n에러가 발생한 파일 경로:")
