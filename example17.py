...
target_path = os.path.join(target_folder, 'file_info11.xlsx')
...
# json parsing 오류 최소하 위해 response text 정제
import re
...
def extract_information(text):
    ...
    번역사의 이름, 이메일, 전화번호, 현 거주지(도시 이름까지만), 자기 소개 개요, 번역 경력년수(명시되어있다면 명시된 년수와 시작년도를 표기하고, 명시되지 않았다면 활동 내역으로 추정한 년수와 시작년도를 표기해줘. 알 수 없다면 '알 수 없음'), 번역 가능한 언어, 통역 가능한 언어, 번역 툴 트라도스 사용가능여부, 주요 학력, 주요 경력, 해외(한국 외) 교육기관에서 공부경험 유무, 그밖에 번역사로서 경쟁력 등을 아래의 출력문 사례처럼 작성해줘.
...
for root, dirs, files in os.walk(source_folder):
    ...
    files = [file for file in files if file not in system_files and excluded_char not in file and (file.lower().endswith('.pdf') or file.lower().endswith('.txt') or file.lower().endswith('.html'))]
    for file in files:
