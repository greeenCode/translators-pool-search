# 폴더 내 조건 맞는 파일의 'File Name', 'File Name2', 'File Extension', 'File Link', 'Last Modified' 저장
        ...
        for file in files:
            if file not in system_files and excluded_char not in file:  # 시스템 파일 및 제외할 문자 포함 파일 제외
                ...
                if file_modified_time > modified_file_date:
                    ...
                    file_data.append((file, file_name, file_extension, file_link, file_modified_time))
    
    # 데이터프레임 생성
    df = pd.DataFrame(file_data, columns=['File Name', 'File Name2', 'File Extension', 'File Link', 'Last Modified'])
