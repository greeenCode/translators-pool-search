...
target_path = os.path.join(target_folder, 'file_info10.xlsx')
...
def extract_information(text):
    ...
    번역사의 이름, 이메일, 전화번호, 현 거주지(도시 이름까지만), 자기 소개 개요, 번역 경력년수(명시되어있다면 명시된 년수와 시작년도를 표기하고, 활동 내역으로 추정한 년수와 시작년도를 표기해줘. 알 수 없다면 '알 수 없음'), 번역 가능한 언어, 통역 가능한 언어, 번역 툴 트라도스 사용가능여부, 주요 학력, 주요 경력, 해외(한국 외) 교육기관에서 공부경험 유무, 그밖에 번역사로서 경쟁력 등을 아래의 출력문 사례처럼 작성해줘.
    ...
    {{
        ...
        연구원, Georgetown University (2006-2009); 연구원, SUNY Upstate Medical University (2004-2006); 
...
for root, dirs, files in os.walk(source_folder):
    ...
    files = [file for file in files if file not in system_files and excluded_char not in file and (file.lower().endswith('.pdf') or file.lower().endswith('.txt'))]
    for file in files:
...
if proceed in ('yes', 'y', ''):
    ...
    for root, dirs, files in os.walk(source_folder):
        ...
        for file in files:
            ...
            if file_modified_time > modified_file_date:
                processed_count += 1
                print(f"{file_count} 개 파일 중 {processed_count} 번째 파일 작업 중 ...")
                try:
                    # pdf, txt, html에 따른 함수 호출
                    ...
                    if extracted_info is not None:
                        ...
                        # Update the 'File Link' to be relative to the Excel file location
                        relative_file_path = os.path.relpath(file_path, start=target_folder)
                        extracted_info['File Link'] = f'=HYPERLINK("{relative_file_path}")'
                        extracted_info['이름'] = f'=HYPERLINK("{relative_file_path}", "{extracted_info["이름"]}")'
    ...
    if file_data:
        ...
        if os.path.exists(target_path):
            ...
            headers = ["이름", "이메일", "전화번호", "거주지", "자기소개", "경력년수", "번역가능언어", "통역가능언어", "트라도스여부", "주요학력", "주요경력", "해외학업유무", "경쟁력", "파일수정일", "File Link"]
            ws.append(headers)
        
        # Find the next available row in the existing worksheet
